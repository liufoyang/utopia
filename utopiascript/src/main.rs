...
fn main() {
    ...
    if arguments.len() > 1 {
        let complile_file = arguments[1].as_str();
        info!("complile_file file {}", complile_file);

        let source_file_result = File::open(complile_file);
        ...
        // write class file
        let mut class_file_name = String::from(complile_file);
